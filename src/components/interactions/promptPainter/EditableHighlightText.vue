...
<div class="grid">
    <div class="col-12 md:col-8 bg-white">
      <div class="editable-container">
        <div
          ...
          ref="contentElement"
          @mouseup="handleMouseUp($event, null)"
        >
          <template
        ...
        <Column field="text" header="Text Segment" style="width: 70%">
          <template #body="{ data }">
            <Textarea
              ...
              :value="data.text.trim().substring(0, 50) + '...'"
              rows="1"
          ...
          <template #editor="{ data }">
            <Textarea
              ...
              autocomplete="off"
              rows="auto"
              @keydown="handleTextareaKeydown($event, data)"
            />
    ...
    <div class="col-12 lg:col-4">
      <div class="prompt-painter control-bar p-2 border-round bg-gray-100">
        ...
        <div class="button-row">
          <Button
            @click="paintAction('expand')"
            label="Expand 2x"
            class="p-button-success"
          />
          <Button
            @click="paintAction('contract')"
            label="Contract 1/2"
            class="p-button-success"
          />
        </div>

        <label class="text-lg" for="specialInstructions"
        ...
        <div class="button-row">
          <Button @click="paintAction('refine')"
           label="Submit"  class="p-button-success" />
        </div>
...
watch(currentCursor, (newCursor, oldCursor) => {
  if (newCursor.segmentIndex !== oldCursor.segmentIndex) {
    ...
    if (
      newCursor.segmentIndex >= 0 &&
      newCursor.segmentIndex < updatedSegments.length
    ) {
...
watch(
  ...
  async (newCursor, oldCursor) => {
    ...
    const spanElement = contentElement.value.querySelector(
      `span[data-uuid="${newCursor.uuid}"]`
    );
    if (spanElement) {
...
function updateTextAndHighlights() {
  if (contentElement.value) {
    const spanElements =
      contentElement.value.querySelectorAll("span.text-content");
...
function updateCursorPosition() {
  ...
  if (selection.rangeCount > 0) {
    ...
    if (
      currentNode === contentElement.value &&
      contentElement.value.childNodes.length > 0
    ) {
      currentNode =
        contentElement.value.childNodes[
          contentElement.value.childNodes.length - 1
        ];
...
function getSelectionRangeWithinSegment(segment) {
  ...
  if (selection.rangeCount > 0) {
    ...
    if (
      range.commonAncestorContainer.parentNode.getAttribute("data-uuid") ===
      segment.uuid
    ) {
